...
function createMap<T extends object, K extends keyof T, P extends keyof T>(
  arr: T[],
  keyProp: K,
  prop?: P
): Map<T[K], T | T[P]> {
  return new Map<T[K], T | T[P]>(
    arr.map((obj): [T[K], T | T[P]] => {
      let key = obj[keyProp];
      let value = prop ? obj[prop] : obj;
      return [key, value];
    })
  );
}
...
// 1
console.log(createMap(collection, "id")); // ok
console.log(createMap(collection, "passport")); // ok
// createMap(collection, "surname"); // error
// createMap(collection, 4); // error

// 2
console.log(createMap(collection, "id", "name")); // ok
console.log(createMap(collection, "passport", "age")); // ok
